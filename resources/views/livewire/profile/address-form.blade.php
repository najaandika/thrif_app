    ...
    <form wire:submit.prevent="save" class="space-y-5">
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <input type="text" id="recipient_name" name="recipient_name" wire:model.defer="recipient_name" class="profile-field h-12" placeholder="Nama lengkap" autocomplete="name" />
            ...
            <div>
                ...
                <input type="text" id="phone" name="phone" wire:model.defer="phone" class="profile-field h-12" placeholder="08xxxxxxxxxx" autocomplete="tel" />
                @error('phone')
                    <p class="mt-2 text-sm font-bold text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea id="address_line" name="address_line" wire:model.defer="address_line" rows="4" class="profile-field leading-6" placeholder="Nama jalan, nomor rumah, kecamatan, kota, dan patokan"></textarea>
            @error('address_line')
                <p class="mt-2 text-sm font-bold text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <button type="submit" class="profile-primary-btn min-h-12 px-6">
                Simpan alamat
            </button>

            <a href="{{ route('landing.products.index') }}" class="profile-secondary-btn min-h-12 px-6">
                Buka katalog
            </a>
        </div>
    </form>
