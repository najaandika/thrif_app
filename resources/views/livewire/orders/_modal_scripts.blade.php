<script>
    document.addEventListener('alpine:init', () => {
        @php $ord = $selectedOrder ?? $order ?? null; @endphp
        Alpine.data('receiptModal', () => ({
            ...
            downloadReceipt() {
                ...
                html2canvas(element, {
                    ...
                    link.download = 'Receipt-' + '{{ $ord->invoice_number ?? "INV" }}' + '.jpg';
            ...
            shareToWa() {
                ...
                @if($ord)
                    ...
                    items += '• {{ addslashes($item->product->name ?? "Item") }} (x{{ $item->quantity }})\n';
                    @endforeach
                @endif

                let message = '*STRUK DIGITAL - {{ strtoupper(\App\Models\Setting::get("shop_name") ?? "THRIF STUDIO") }}*\n';
                message += '--------------------------------\n';
                message += 'No. Invoice: {{ $ord->invoice_number ?? "-" }}\n';
                message += 'Tanggal: {{ optional($ord->created_at)->format("d/m/Y H:i") ?? "-" }}\n\n';
                message += '*Detail Belanja:*\n';
                message += items;

                @if(($ord->discount ?? 0) > 0)
                message += '\nDiskon: - Rp {{ number_format($ord->discount, 0, ",", ".") }}';
                @endif

                message += '\n--------------------------------\n';
                message += '*TOTAL: Rp {{ number_format($ord->total_price ?? 0, 0, ",", ".") }}*\n';
                message += '--------------------------------\n';
