        ...
        <tbody class="admin-tbody">
            @forelse ($products as $product)
                <tr class="admin-tr">
                    ...
                    <td class="admin-td">
                        <div class="text-primary">Rp {{ number_format($product->price, 0, ',', '.') }}</div>
                    </td>
                    <td class="admin-td">
                        <span class="stock-badge {{ $product->stock > 0 ? 'stock-badge-available' : 'stock-badge-low' }}">
                    ...
                    <td class="admin-td">
                        <span class="text-secondary">{{ $product->sizes->pluck('size')->join(', ') ?: '-' }}</span>
                    </td>
                    <td class="admin-td">
                    ...
                    <td class="admin-td text-right space-x-2">
                        ...
                        <button type="button" onclick="confirmDelete({{ $product->id }})" class="btn-action-delete">
                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
