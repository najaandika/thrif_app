<div class="h-full rounded-3xl border border-slate-300/80 dark:border-slate-700 bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800/95 dark:from-slate-900/95 dark:via-slate-800/95 dark:to-slate-800/90 p-5 sm:p-6 space-y-4 relative overflow-hidden opacity-0 translate-y-8" id="kontak" data-animate data-section="contact">
    ...
    <div class="flex flex-col gap-2.5 text-[13px] relative z-10">
          ...
          <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $shopPhone) }}"
              class="group inline-flex items-center gap-2.5 rounded-2xl bg-slate-900/90 hover:bg-slate-800 border border-slate-700 px-4 py-3 text-slate-50 font-semibold shadow-lg shadow-slate-950/40 transition-all duration-300 hover:scale-[1.02]">
            ...
            <div class="flex-1 text-left">
                ...
                <div class="font-semibold tracking-wide">{{ $shopPhone }}</div>
            </div>
        ...
        <a href="{{ $socialInstagram }}"
           ...
           class="group inline-flex items-center gap-2.5 rounded-2xl border border-slate-700 bg-slate-900/80 px-4 py-3 text-slate-50 font-semibold shadow-lg shadow-slate-950/40 transition-all duration-300 hover:scale-[1.02] hover:bg-slate-800">
            ...
            <div class="flex-1 text-left">
                ...
                <div class="font-semibold tracking-wide">{{ parse_url($socialInstagram, PHP_URL_PATH) ?: '@yourshop' }}</div>
            </div>
            <svg class="w-5 h-5 text-slate-300 transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
        ...
        @if($socialTiktok)
          <a href="{{ $socialTiktok }}"
            ...
            <div class="flex-1 text-left">
                ...
                <div class="font-semibold tracking-wide">{{ parse_url($socialTiktok, PHP_URL_PATH) ?: '@yourshop' }}</div>
            </div>
            <svg class="w-5 h-5 text-slate-300 transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
