<section class="bg-white border border-gray-200 rounded-3xl p-6 space-y-5">
    <p class="{{ $labelClass }}">Produk</p>
    <div class="space-y-4">
        <div class="relative rounded-2xl overflow-hidden border border-gray-100">
            @if ($product->image)
                <img src="{{ media_url($product->image) }}" alt="{{ $product->name }}" class="w-full h-44 object-cover">
            @else
                <div class="w-full h-44 bg-gradient-to-br from-slate-200 via-slate-100 to-slate-300 flex items-center justify-center text-xs text-gray-500">
                    Foto produk menyusul
                </div>
            @endif
            @if ($product->is_on_sale)
                <div class="absolute top-2 left-2 bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                    -{{ $product->discount_percent }}% OFF
                </div>
            @endif
        </div>
        <div class="space-y-2">
            <h1 class="text-2xl font-semibold text-gray-900 leading-tight">{{ $product->name }}</h1>
            <p class="text-sm text-gray-500">Kategori: {{ $product->category ?? 'Tanpa kategori' }}</p>
            <p class="text-sm text-gray-500">Kondisi: {{ $product->condition_label }}</p>
            @if ($product->sizes->count() === 1)
                <p class="text-sm text-gray-500">Ukuran: <span class="font-semibold text-gray-700">{{ $product->sizes->first()->size }}</span></p>
            @endif
            @if ($product->description)
                <div class="pt-2 border-t border-gray-100">
                    <p class="text-xs font-semibold text-gray-700 mb-1">Deskripsi:</p>
                    <p class="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{{ strip_tags($product->description) }}</p>
                </div>
            @endif
            @if ($product->is_on_sale)
                <div class="pt-2">
                    <p class="text-lg text-gray-400 line-through">{{ rupiah($product->price) }}</p>
                    <p class="text-3xl font-bold text-red-500">{{ rupiah($product->final_price) }}</p>
                    <p class="text-xs text-red-500 font-medium">Hemat {{ rupiah($product->price - $product->final_price) }}!</p>
                </div>
            @else
                <p class="text-3xl font-bold text-green-600 pt-2">{{ rupiah($product->price) }}</p>
            @endif
        </div>
    </div>
    <div class="rounded-2xl bg-gray-50 p-4 text-xs text-gray-600 space-y-1">
        <p>Stok tersedia: <span class="font-semibold">{{ $product->sizes->isNotEmpty() ? $product->total_stock : $product->stock }}</span></p>
        <p>Status: <span class="font-semibold text-gray-900">{{ $product->is_available ? 'Ready to ship' : 'Sold Out' }}</span></p>
    </div>
    <div class="rounded-2xl border border-dashed border-gray-300 p-4 text-xs text-gray-500">
        Kamu bisa review kembali order ini setelah submit. Admin akan menghubungi lewat kontak yang kamu isi.
    </div>
</section>




